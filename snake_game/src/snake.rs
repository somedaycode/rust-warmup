...
const SNAKE_COLOR: Color = [0.0, 0.0, 1.0, 0.8];
...
impl Snake {
    ...
    pub fn draw(&self, context: &Context, graphic2d: &mut G2d) {
        for block in &self.body {
            draw_block(SNAKE_COLOR, block.x, block.y, context, graphic2d);
        }
    }
    ...
    pub fn move_forward(&mut self, direction: Option<Direction>) {
        match direction {
            Some(direct) => self.direction = direct,
            None => (),
        }
    ...
    pub fn next_head(&self, direction: Option<Direction>) -> (i32, i32) {
        ...
        match direction {
            Some(direct) => moving_direction = direct,
            None => {}
        }
